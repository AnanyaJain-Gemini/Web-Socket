...
import { Coordinates, UserMovedPayload } from "../server/types";

const socket = io();

const map = new maplibregl.Map({
  container: "map", 
  style: "https://api.maptiler.com/maps/streets/style.json?key=kkXA2icqeH8jsEhb4cQp",
  center: [78, 20],
  zoom: 5,
});

const userMarkers: Record<string, maplibregl.Marker> = {};
...
socket.on("user-moved", ({ id, coords }: UserMovedPayload) => {
  if (!userMarkers[id]) {
    const img = document.createElement("img");
    img.src = "https://cdn-icons-png.flaticon.com/512/684/684908.png";
    img.style.width = "30px";
